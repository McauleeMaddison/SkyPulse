...
Use SkyPulse 1.0.0 (4) in the open Xcode iOS Simulator. The final icon has two wings and a blue-violet cosmic background. Existing scores and crystals were preserved during installation.
...
4. **Prepare the signed iPhone build.** Use `Builds/iOS-device-background-4/Unity-iPhone.xcodeproj` only if source/icon hashes still match `BETA_VERIFICATION.json`; otherwise export to a new empty folder. Choose the correct signing team and a generic iOS device destination, archive in Release, and validate in Xcode Organizer. A Simulator app cannot be uploaded. Increase the build number if App Store Connect has already received build 4. Check the final icon, bundle ID, portrait orientation, encryption declaration and privacy manifests. Preserve symbols for crash reports.
...
For build 4, open mobile/Builds/iOS-simulator-background-4/Unity-iPhone.xcodeproj and select iPhone 17 (the currently tested Simulator). Do not run an older export, which would reinstall earlier tuning. The older Desktop/SkyPulse_iOS_Simulator project is stale and must not be used to reinstall the candidate.
